    ...
    <a-card title="成员管理" :bordered="false">
      <a-table :columns="columns" :data-source="dataSource" :pagination="false">
        <template #bodyCell="{ text, record, index, column }">
         <template v-for="col in ['name', 'age', 'address']" :key="col">
...
const newMember = () => {
  ...
  editableData[dataSource.value[dataSource.value.length - 1].key] = cloneDeep(
    dataSource.value.filter(item => dataSource.value[dataSource.value.length - 1].key === item.key)[0]
  )
}
const handSubmit = () => {
  loading.value = true
